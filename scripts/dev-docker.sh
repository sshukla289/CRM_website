...
echo "[dev-docker] Starting Next dev on ${BIND_HOST}:${PORT}"
...
case "$NEXT_HELP" in
  *"--webpack"*)
    ...
    exec "$NEXT_BIN" dev --webpack -H "$BIND_HOST" -p "$PORT"
    ;;
  *"--no-turbo"*)
    echo "[dev-docker] Using --no-turbo for more reliable hot reload."
    exec "$NEXT_BIN" dev --no-turbo -H "$BIND_HOST" -p "$PORT"
    ;;
esac

echo "[dev-docker] Running default dev server."
exec "$NEXT_BIN" dev -H "$BIND_HOST" -p "$PORT"
