...
export function getIndustrySlug(industry) {
  return slugify(getIndustrySeoTitle(industry));
}

export function getIndustrySeoTitle(industry) {
  return `Top ${industry?.name || ""} CRM in India`;
}

export function getIndustryLegacySlug(industry) {
...
export function getIndustryBySlug(slug) {
  return (
    industries.find(
      (industry) =>
        getIndustrySlug(industry) === slug || getIndustryLegacySlug(industry) === slug
    ) || null
  );
}

export function getRelatedIndustries(slug, limit = 3) {
  const currentIndustry = getIndustryBySlug(slug);

  return industries
    .filter((industry) => industry.name !== currentIndustry?.name)
    .slice(0, limit);
}
